...
type Data = { settings:{libraryName:string; schoolYear:string; loanDays:number; theme:string}; students:Student[]; books:Book[]; loans:Loan[]; today:string };
...
async function request(action:string, payload:Record<string,unknown>={}) {
  ...
  return result as {error?:string;inserted?:number;skipped?:number};
}

function downloadExcel(filename:string, sheetName:string, rows:Record<string,unknown>[]) {
  const sheet=XLSX.utils.json_to_sheet(rows); const book=XLSX.utils.book_new(); XLSX.utils.book_append_sheet(book,sheet,sheetName); XLSX.writeFile(book,filename);
}
async function readExcel(file:File) {
  const data=await file.arrayBuffer(); const workbook=XLSX.read(data); const sheet=workbook.Sheets[workbook.SheetNames[0]]; return XLSX.utils.sheet_to_json<Record<string,unknown>>(sheet,{defval:""});
}
function pick(row:Record<string,unknown>, names:string[]) { const entry=Object.entries(row).find(([key])=>names.includes(key.trim().toLocaleUpperCase("tr"))); return entry?.[1]??""; }
...
export default function LibraryApp() {
  ...
  return <div className="shell" data-theme={data.settings.theme||"forest"}>
    <aside>
...
function Books({books,active,busy,act}:{books:Book[];active:Loan[];busy:boolean;act:(a:string,p:Record<string,unknown>,s:string)=>Promise<void>}) {
  ...
  const exportRows=()=>downloadExcel("kitap-listesi.xlsx","Kitaplar",books.map(x=>({DN:x.inventoryNo,ISBN:x.isbn,"KİTAP ADI":x.title,YAZARI:x.author,YAYINEVİ:x.publisher,KATEGORİ:x.category,TÜR:x.genre,"RAF NO":x.shelf,"DEWEY KODU":x.dewey,"SAYFA SAYISI":x.pages})));
  const importFile=async(file:File)=>{const raw=await readExcel(file);const rows=raw.map(row=>({inventoryNo:pick(row,["DN","DEMİRBAŞ NO"]),isbn:pick(row,["ISBN","ISBN NO"]),title:pick(row,["KİTAP ADI","KITAP ADI"]),author:pick(row,["YAZARI","YAZAR"]),publisher:pick(row,["YAYINEVİ","YAYINEVI"]),category:pick(row,["KATEGORİ","KATEGORI"]),genre:pick(row,["TÜR","TUR"]),shelf:pick(row,["RAF NO","RAF"]),dewey:pick(row,["DEWEY KODU","DEWEY"]),pages:pick(row,["SAYFA SAYISI","SAYFA"])}));await act("importBooks",{rows},"Kitap Excel dosyası işlendi; mükerrer ve eksik kayıtlar atlandı.")};
  return <section><PageHead title="Kitaplar" text="Katalog kayıtlarını arayın, raf durumunu izleyin ve yeni kitap ekleyin." action={<div className="actions"><button className="secondary" onClick={exportRows}>Dışa aktar</button><label className="import-btn">İçe aktar<input type="file" accept=".xlsx,.xls,.csv" onChange={e=>{const file=e.target.files?.[0];if(file)void importFile(file);e.target.value=""}}/></label><button className="primary" onClick={()=>{setEditing(null);setOpen(!open)}}>+ Kitap ekle</button></div>}/>{open&&<BookForm busy={busy} initial={editing} submit={async p=>{await act(editing?"updateBook":"addBook",editing?{...p,id:editing.id}:p,editing?"Kitap güncellendi.":"Kitap kataloğa eklendi.");setOpen(false);setEditing(null);}} onDelete={editing?async()=>{if(confirm("Bu kitabı silmek istediğinize emin misiniz?")){await act("deleteBook",{id:editing.id},"Kitap silindi.");setOpen(false);setEditing(null);}}:undefined}/>}<Search value={q} setValue={setQ} placeholder="Kitap adı, yazar, DN veya ISBN ile ara"/><div className="book-grid">{shown.map(book=><article className="book" key={book.id}><div className="cover">{book.title.slice(0,1)}</div><div><span className={active.some(x=>x.bookId===book.id)?"badge red":"badge green"}>{active.some(x=>x.bookId===book.id)?"Ödünçte":"Rafta"}</span><h3>{book.title}</h3><p>{book.author}</p><small>{book.inventoryNo} · {book.shelf||"Raf belirtilmedi"}</small><button className="edit-link" onClick={()=>{setEditing(book);setOpen(true)}}>Düzenle</button></div></article>)}</div>{!shown.length&&<Empty text="Aramanızla eşleşen kitap bulunamadı."/>}</section>;
}
