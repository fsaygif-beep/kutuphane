...
export default function LibraryApp() {
  ...
  return <div className="shell">
    ...
    <main>
      ...
      {tab==="settings"&&<Settings config={data.settings} empty={!data.books.length&&!data.students.length} busy={busy} act={act}/>} 
    </main>
  </div>;
}
...
function Books({books,active,busy,act}:{books:Book[];active:Loan[];busy:boolean;act:(a:string,p:Record<string,unknown>,s:string)=>Promise<void>}) {
  ...
  return <section><PageHead title="Kitaplar" text="Katalog kayıtlarını arayın, raf durumunu izleyin ve yeni kitap ekleyin." action={<button className="primary" onClick={()=>setOpen(!open)}>+ Kitap ekle</button>}/>{open&&<BookForm busy={busy} submit={async p=>{await act("addBook",p,"Kitap kataloğa eklendi.");setOpen(false);}}/>}<Search value={q} setValue={setQ} placeholder="Kitap adı, yazar, DN veya ISBN ile ara"/><div className="book-grid">{shown.map(book=><article className="book" key={book.id}><div className="cover">{book.title.slice(0,1)}</div><div><span className={active.some(x=>x.bookId===book.id)?"badge red":"badge green"}>{active.some(x=>x.bookId===book.id)?"Ödünçte":"Rafta"}</span><h3>{book.title}</h3><p>{book.author}</p><small>{book.inventoryNo} · {book.shelf||"Raf belirtilmedi"}</small></div></article>)}</div>{!shown.length&&<Empty text="Aramanızla eşleşen kitap bulunamadı."/>}</section>;
}
...
function Students({students,loans,busy,act}:{students:Student[];loans:Loan[];busy:boolean;act:(a:string,p:Record<string,unknown>,s:string)=>Promise<void>}) {
  ...
  return <section><PageHead title="Öğrenciler" text="Öğrenci numarası ve sınıf bilgileriyle üyeleri yönetin." action={<button className="primary" onClick={()=>setOpen(!open)}>+ Öğrenci ekle</button>}/>{open&&<StudentForm busy={busy} submit={async p=>{await act("addStudent",p,"Öğrenci kaydedildi.");setOpen(false);}}/>}<Search value={q} setValue={setQ} placeholder="Öğrenci no, ad soyad veya sınıf ile ara"/><div className="table-card"><table><thead><tr><th>Öğrenci</th><th>Sınıf</th><th>İletişim</th><th>Okuduğu Kitap</th></tr></thead><tbody>{shown.map(x=><tr key={x.id}><td><strong>{x.fullName}</strong><small>{x.studentNo}</small></td><td><span className="badge blue">{x.grade}</span></td><td>{x.contact||"—"}</td><td>{loans.filter(l=>l.studentId===x.id&&l.returnedAt).length}</td></tr>)}</tbody></table></div></section>;
}
